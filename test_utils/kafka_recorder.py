...
def main():
    args = parse_args()
    logger.info(
        f"Starting Kafka recorder with broker {args.broker}, topic {args.topic}"
    )
    ...
    try:
        output_file = record_messages(
            consumer, args.topic, args.output_dir, args.max_messages, args.timeout
        )
