...
def parse_args():
    parser = argparse.ArgumentParser(description="Replay recorded Kafka messages to a topic")
    parser.add_argument(
...
def replay_messages(producer, input_file, target_topic, rate, start_from, count):
    ...
    if start_from > 0:
        if start_from >= total_messages:
            logger.error(f"Start index {start_from} exceeds total message count {total_messages}")
            return
        messages = messages[start_from:]
        logger.info(f"Starting from message {start_from}, {len(messages)} messages remaining")
    ...
    try:
        for i, msg_data in enumerate(messages):
            ...
            headers = [(k, v.encode('utf-8') if v is not None else None) for k, v in headers]
            
            # Control replay rate based on original message timestamps
            current_timestamp = msg_data.get("timestamp")
            if last_timestamp is not None and current_timestamp is not None and rate > 0:
                time_diff = (current_timestamp - last_timestamp) / 1000.0  # ms to seconds
